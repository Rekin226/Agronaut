...
def _strip_running_headers(docs, min_fraction: float = 0.15):
    ...
    for d in docs:
        seen = {_normalise_for_repetition(ln) for ln in d.page_content.splitlines() if ln.strip()}
        for line in seen:
            if line:
                counts[line] += 1
    threshold = max(3, int(len(docs) * min_fraction))
    furniture = {ln for ln, c in counts.items() if c >= threshold and len(ln) > 8}
    if not furniture:
        return docs
    for d in docs:
        d.page_content = "\n".join(
            ln for ln in d.page_content.splitlines()
            if _normalise_for_repetition(ln) not in furniture)
...
def _looks_like_contents_page(text: str) -> bool:
    ...
    if len(lines) < 6:
        return False
    numbered = sum(1 for ln in lines if re.search(r"\s\d{1,4}\s*$", ln))
...
def _label_pdf_chapters(docs):
    ...
    current = ""
    for d in docs:
        lines = list(d.page_content.splitlines())
